  ...
  eachCluster(item, i) {
    ...
    return (
      <tr key={i} className="trC2">
        <td className="radioJoin"><div><input type="radio" id={"cluster" + item.name} value={"cluster" + item.name} onChange={handleClusterChange}/></div></td>
        <td className="divForClusterImg"><div><img className="clusterImg" src="icon/clusters.png"/></div></td>
        <td className="clusterName"><div><font> {item.name} </font></div></td>
        <td className="clusterIp"><div><font> {item.link.slice(0, -5)} </font></div></td>
      </tr>
      ...
      if(this.props.mode === "info") {
          ...
          return (
            <div id = "body">
                ...
                <div className="closePBC">
                  <div className="closePopupButton" onClick={focusLeave}>{mainPage_lang.Infotab_buttonClose}</div>
                </div>
            </div>
          );
        }
            ...
            <div id = "body">
              <div id="namewin">
                ...
                <div id = "block">
                    ...
                    <table>
                      <tbody>
                        <tr><th className="thApp">{mainPage_lang.Infotab_SN}</th><th className="thApp">{mainPage_lang.Infotab_SysType}</th><th className="thApp">{mainPage_lang.Infotab_IP}</th></tr>
                        <tr><td className="tdApp">X746</td><td className="tdApp">SAN</td><td className="tdApp">192.168.1.1</td></tr>
                        <tr><td className="tdApp">X741</td><td className="tdApp">SAN</td><td className="tdApp">192.168.1.2</td></tr>
                      </tbody>
                    </table>
                </div>
                <div className="closePBC">
        ...
        return(
          <div id = "body">
            ...
            <div id="avaliable">
              <div id="search">
                <div id="nameBlock">{mainPage_lang.formatString(mainPage_lang.Infotab_AvailCl, this.state.filterClusters.length)}</div>
                <div id = "line"></div>
                <input type="text" id="searchLine" style={{margin: "30px 0px"}} onChange={this.filterClusterList} placeholder={mainPage_lang.Infotab_PlaceHolder}>
                </input>
                <table id="clustersList">
                  <thead><tr className="trCl"><th colSpan={3} className="hCl">{mainPage_lang.Infotab_name}</th><th className="hC2">{mainPage_lang.Infotab_IP}</th></tr></thead>
            ...
            <div id = "block2">
              <div id="nameBlock">{mainPage_lang.formatString(mainPage_lang.Infotab_selectedAppliances, 1)}</div>
              <div id = "line"></div>
              <table id="selectedApplianceForJoin">
                <tbody>
                  <tr><th className="thApp">{mainPage_lang.Infotab_SN}</th><th className="thApp">{mainPage_lang.Infotab_SysType}</th><th className="thApp">{mainPage_lang.Infotab_IP}</th></tr>
                  <tr><td className="tdApp">{this.props.ssn}</td><td className="tdApp">{this.props.type}</td><td className="tdApp">{ip}</td></tr>
                </tbody>
              </table>
            </div>
            <div className="closePBC">
              <div className="closePopupButton" onClick={focusLeave}>{mainPage_lang.Infotab_buttonCancel}</div>
              <div className="joinButton" onClick = {this.clickOnJoinCluster}>{mainPage_lang.Infotab_buttonJoin}</div>
            </div>
          </div>
        );
      }
