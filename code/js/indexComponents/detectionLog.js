...
class DetectionLog extends React.Component {
  ...
  eachLog(item, i) {
    return (
      <tr key={i}>
        <td><div><font> {item.name + " " + item.status} </font></div></td>
        <td><div><font> {item.link.slice(7)} </font></div></td>
        <td><div><font> {item.state} </font></div></td>
        <td><div><font> {item.type} </font></div></td>
        <td><div><font> {item.date} </font></div></td>
      </tr>
    );
  }
        ...
        <div id="logBox">
          ...
          <div id="logShow">
            ...
            <div id="whiteSpaceForLogTable">
              <div className="saveLogButton" onClick={this.saveLogs.bind(this, "text123", "logs.txt", "text/plain")}>
              ...
              <table id="detectionLogTable">
                <thead>
                  <tr>
                    <th>{mainPage_lang.Logs_Name}</th><th>{mainPage_lang.Logs_Addr}</th><th>{mainPage_lang.Logs_State}</th><th>{mainPage_lang.Logs_Type}</th><th>{mainPage_lang.Logs_Time}</th>
                  </tr>
                </thead>
          ...
          <div id="logShow">
            ...
            <div id="whiteSpaceForLogTable">
              ...
              <table id="eventLogTable">
                <thead>
                  <tr>
                    <th>{mainPage_lang.Logs_Name}</th><th colSpan="3">{mainPage_lang.Logs_Time}</th>
                  </tr>
                </thead>
                <tbody id="detectionLogTableBody">
                </tbody>
